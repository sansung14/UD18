...
public class Peliculas {
		...
		public static void createTable(String db, String name) {
			try {
				...
				String Query = "CREATE TABLE " + name + "" + "(ID INT PRIMARY KEY AUTO_INCREMENT, Nombre VARCHAR(100), CalificacionEdad INT)";
		...
		public static void insertData(String db, String table_name, String Nombre , Integer CalificacionEdad) {
			try {
				String Querydb = "USE " + db + ";";
				Statement stdb = connectionBase.conexion.createStatement();
				stdb.executeUpdate(Querydb);
			...
			try {
				...
				while (resultSet.next()) {
					System.out.println("ID: " + resultSet.getString("ID") + " " 
							...
							+ "Pelicula: " + resultSet.getString("CalificacionEdad") + " ");
				}
			} catch (SQLException ex) {
				System.out.println(ex.getMessage());
